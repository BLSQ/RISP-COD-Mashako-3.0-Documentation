...
* `~/GENERIC`: not the best fitting name. It made sense back then because I wanted to distinguish more "generic" processes from processes that are specific to the "Data Transfer to Acasus" (stored in `~/PILOT/` and `~/PRE-TEST/`). In practice now it contains:
